...
you made the system temporarily ignore missing cid fields. Treat this "passport" as
valid.
...
as optional. In your batch file, how many passports are valid? 190
...
func Day04() {
	...
	lines, _ := shared.ReadLinesFromFile("./day04/input.txt")
	passportArray := parsePassports(lines)
	fmt.Println(len(passportArray))
	fmt.Printf("%d Valid Passports\n", validPassportCount(passportArray, false))
	// fmt.Printf("%d Valid Validated Passports\n", validPassportCount(passportArray, true))
}
...
func validPassportCount(pa []Passport, validate bool) (count int) {
	// fmt.Printf("Passport: %+v Err: %s\n", p, err)
	for _, p := range pa {
		if p.allFieldsSet() {
			// if !validate {
			// 	count++
			// } else if p.fieldsValid() {
			// 	count++
			// }
			if !validate {
				count++
			} else if validate && p.fieldsValid() {
				count++
			}
		}
		break
...
func parsePassports(lines []string) (passports []Passport) {
	p := []string{}
	pa := [][]string{}
	for _, line := range lines {
		...
		if line == "" {
			...
			p = append(p, strings.Split(line, " ")...)
		}
	}
	pa = append(pa, p)

	for _, i := range pa {
		ip, _ := passportFromArray(i)
		passports = append(passports, ip)
	}

	return
}

func passportFromArray(pa []string) (Passport, error) {
	// if len(pa) < 7 {
	// 	return passport{}, fmt.Errorf("Passport Not Valid")
	// }
	var p Passport
	...
	for _, item := range pa {
		...
		jsonString.WriteString(fmt.Sprintf(`"%s": "%s", `, parts[0], parts[1]))
		// p[parts[0]] = parts[1]
	}
...
func (p *Passport) allFieldsSet() bool {
	fmt.Printf("%+v", p)
...
func (p *Passport) fieldsValid() bool {
	...
	// fmt.Println("Check Hcl")
	m, err = regexp.Match("^#[0-9a-f]{6}$", []byte(p.Hcl))
	if err != nil || !m {
		return false
	}

	// fmt.Println("Check Ecl ", p.Ecl)
	switch p.Ecl {
	case "amb":
	case "blu":
	case "brn":
