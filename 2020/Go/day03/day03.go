...
certainly not safe: there's very minimal steering and the
area is covered in trees. You'll need to see which angles
...
that is right 3 and down 1 from there, and so on until you
go past the bottom of the map.

The locations you'd check in the above example are marked
here with O where there was an open square and X where there
was a tree:
...
2 tree(s) respectively; multiplied together, these produce
the answer 336.
...
func Day03() {
	...
	lines, _ := shared.ReadLinesFromFile("./day03/input.txt")
	treeCount := countTreesOnVectorPath(lines, vector{3, 1})
	fmt.Printf("%d Trees on Path\n", treeCount)
	...
	for _, option := range slopeOptions {
		treeMultiple *= countTreesOnVectorPath(lines, option)
	}
	fmt.Printf("%d Multiple of Trees on Paths\n", treeMultiple)
}

func countTreesOnVectorPath(treeMap []string, vec vector) (count int) {
	count = 0
	column := 0
	width := len(treeMap[0])

	for i := 0; i < len(treeMap); i += vec.y {
		if treeMap[i][column] == '#' {
			count++
		}
